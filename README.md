...
- **Виджет на рабочем столе** (включён по умолчанию) — полупрозрачная
  ...
  - **авто-ориентация** — у левого/правого края экрана виджет стоит
    вертикально (кружки в столбик), у верхнего/нижнего края — поворачивается
    горизонтально (кружки в ряд). В середине экрана сохраняется последняя
    ориентация (по умолчанию вертикальная). Ориентация определяется
    автоматически после перетаскивания к краю;
  ...
  - **скрывать виджет при простое** (пункт меню, доступен только когда
    ...
    ушёл с виджета, он уплывает за ближайший край экрана, оставляя видимым
    маленький «огрызок» фона. Стоит навести курсор на этот огрызок — и
    виджет выплывает обратно. Пока курсор на виджете, он не прячется;
