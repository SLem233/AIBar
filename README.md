...
- **вклад в период считается по посуточным срезам** (`daily_activity`):
  сессия, начатая до окна, приносит только работу внутри окна, поэтому KPI,
  «Проекты», «Проект × агент» и сумма столбиков графика сходятся между собой
  (проверяется тестами `tests/test_compute_window.py`, которые гоняют
  настоящий JS страницы через node). У старых сессий без срезов берётся итог
  целиком, если последняя активность попала в окно — иначе они выпали бы
  из статистики совсем. Модели — из `daily_models` там, где они есть;
...
- два блока-напоминания внизу страницы, оба по отметкам AgentPulse в ledger:
  «Материал для ретроспективы» (проекты с сессиями новее последней
  `/session-retrospective`) и «Обзор карточек отстал» (карточки, чьи «Инсайты»
  и «Паттерны» изменились после последнего `/cards-review`). Второй считается
  по хешу этих двух секций, а не по времени файла: `agentpulse update`
  переписывает метрики во всех карточках каждый прогон, и напоминание по mtime
  не гасло бы никогда. Правило продублировано из `agentpulse.cards_review` —
  меняешь список секций там, поправь `_REVIEW_SECTIONS` здесь;
- агенты не захардкожены: колонки «Проект × агент» и карточки агентов
  строятся по данным ledger (Claude Code, Codex, Cursor, …);
- вёрстка списков единая: класс `.row` на все перечни страницы («Проекты»,
  категории работы, инструменты, типы задач, модели). Шкала — не отдельная
  полоса под текстом, а окрашенный отрезок нижней линейки строки, поэтому
  строки со шкалой и без неё одной высоты и стоят на общей сетке. Шкал две и
  они означают разное: где рядом напечатан процент (категории) — доля от
  общего, чтобы полоса совпадала с числом; где напечатаны часы и токены
  (проекты) — доля от лидера, иначе полтора десятка проектов дают одни штрихи.
  Строка KPI на flex, а не на сетке фиксированных колонок: держит 5–7 плиток
  в один ряд, кегль значения считается от ширины плитки;
- секция **«Чем занимались»** отвечает не «сколько», а «что»: вызовы
  инструментов по категориям работы (оболочка, правки, чтение, поиск, сеть,
  делегирование, MCP), топ инструментов, распределение сессий по глубине
  рассуждений и режиму одобрения, плюс счётчик компактификаций контекста как
  метрика трения. Категории сравнимы между агентами (`Bash` у Claude Code и
  `exec` у Codex — одна «оболочка»), поэтому разрез общий; сырые имена
  показываются рядом, MCP-имена сокращаются до «сервер: инструмент». Работа
  субагентов учитывается, как и их токены. Источники — `daily_tools` и
  колонки `compactions`/`effort`/`approval_mode` в ledger; на ledger, собранном
  до этой миграции, секция просто не показывается (колонки читаются через
  `PRAGMA table_info`, страница строится как раньше);
- **профиль работы по проекту**: селектор в заголовке секции сужает её до
  одного проекта — категории, инструменты, компакты, effort и режим одобрения.
  Профили заметно различаются (у проекта с браузерной автоматизацией MCP
  ~57%, у инфраструктурного — оболочка ~50%), поэтому разрез полезнее общего
  среднего. Проект определяется по строке сессии, а не по колонке дневного
  среза: имена совпадают с остальной страницей (включая «Вне реестра»), а
  субагент наследует проект родителя. Фильтр действует только на эту секцию —
  KPI и «Проекты» остаются общими. Если у выбранного проекта нет вызовов в
  новом периоде, секция не обнуляется, а возвращается ко «всем проектам».
...
`$потрачено / $лимит`; обратный отсчёт идёт до конца периода. Знаменитое
«Weekly limit: N%» из `grok /usage` снаружи недоступно: TUI получает его по
WebSocket-реле, которое чужому токену отвечает «3000 Unauthorized», а
заголовков `x-ratelimit-*` у чат-эндпоинта нет вовсе. Когда биллинг не
отвечает, в ошибке об этом честно написано. API-ключ XAI провайдеру бесполезен:
публичный `api.x.ai` лимиты по ключу не отдаёт.
...
последних данных), `http_status` провайдеров, разбор ответов Kimi, Grok и
Copilot, обмен токенов Claude и Grok (включая 403 как гео-блок, упавшее реле
без лишней ротации токена и выключенный обмен), индикацию паузы, сворачивание
виджета у края, ориентацию по краю, изменение размера за рамку и отсчёт на
плитке в UI (offscreen-Qt), а также агрегацию статистики AgentPulse — эти тесты
выполняют JS сгенерированной страницы в node (`tests/compute_probe.js`) и
пропускаются, если node не установлен.
