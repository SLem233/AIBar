...
- **Виджет на рабочем столе** (включён по умолчанию) — полупрозрачная
  ...
  - **скрывать виджет при простое** (пункт меню, доступен только когда
    «Виджет поверх окон» включён) — через 5 секунд после того, как курсор
    ушёл с виджета, он заезжает за ближайший край экрана, оставляя снаружи
    ~15% своей рамки как «закладку». Стоит навести курсор на эту закладку —
    виджет выплывает обратно и пристыковывается к краю экрана. Виджет при
    этом не меняет размер и цвет — только уезжает за границу экрана. Пока
    курсор на виджете, он не прячется;
  - **масштаб** — пункт «Масштаб» в контекстном меню с ползунком 50–150%
    регулирует размер виджета и панели (по умолчанию 100%);
  - на виджете показывается время до сброса ближайшего 5-часового окна
    лимита (`↺ 1ч 22м`);
